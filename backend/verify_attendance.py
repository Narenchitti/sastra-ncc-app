...
async def run_tests():
    ...
    # 5. Test Corporal (CPL) authorization
    print("\nTesting Corporal (CPL) authorization...")
    mock_cpl = {"sub": str(uuid.uuid4()), "role": "cadet", "rank": "Corporal"}
    bulk_data_cpl = {
        "eventId": camp_id,
        "markedBy": "Cpl. Test Marker",
        "records": [
            {"userId": cadet_id, "status": "Present"}
        ]
    }
    res_cpl = await submit_bulk_attendance(bulk_data_cpl, current_user=mock_cpl)
    assert res_cpl["success"] is True, "Corporal should be authorized to mark attendance!"
    print("✅ Corporal authorization verified.")

    # 6. Test assigned Permission Manager authorization
    print("\nTesting assigned Permission Manager authorization...")
    ano_id = "a0000000-0000-0000-0000-000000000000"
    await database.set_permission_manager(cadet_id, ano_id)
    mock_pm = {"sub": cadet_id, "role": "cadet", "rank": "Cadet"}
    bulk_data_pm = {
        "eventId": camp_id,
        "markedBy": "PM Test Marker",
        "records": [
            {"userId": cadet_id, "status": "Present"}
        ]
    }
    res_pm = await submit_bulk_attendance(bulk_data_pm, current_user=mock_pm)
    assert res_pm["success"] is True, "Permission Manager should be authorized to mark attendance!"
    print("✅ Permission Manager authorization verified.")

    # 7. Test Permission status camp increment
    print("\nTesting Permission status camp count increment...")
    bulk_data_perm = {
        "eventId": camp_id,
        "markedBy": "Capt. ANO Officer",
        "records": [
            {"userId": cadet_id, "status": "Permission"}
        ]
    }
    res_perm = await submit_bulk_attendance(bulk_data_perm, current_user=mock_ano)
    assert res_perm["success"] is True, "Bulk attendance submission for Permission status failed!"
    
    users = await database.get_users()
    updated_cadet_perm = next((u for u in users if u.id == cadet_id), None)
    print(f"Updated cadet camp count (Permission): {updated_cadet_perm.camp_count}")
    assert updated_cadet_perm.camp_count == 1, f"Expected camp count 1 (for Permission), got {updated_cadet_perm.camp_count}"
    print("✅ Permission status camp count increment verified.")

    # 8. Mark cadet as Absent for the Camp event
    ...
    # 9. Verify camp_count decremented back to 0
    users = await database.get_users()
    ...
    # Reset permission manager in config
    await database.set_permission_manager(None, "ano-id")
